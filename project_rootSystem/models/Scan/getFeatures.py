...
def GetFeatures(all_paths,rootFeatures,denoise_image):
    ...
    # 扫描图片的像素与世界距离转换的计算公式：每像素代表的尺寸（mm）= 25.4/DPI
    DPI = 200
    pixel_size_mm = float(25.4 / DPI)
    ...
    coords =np.where(img_arr == 255) # 提取像素值为255的坐标
    coords_normal = list(zip(coords[0], coords[1]))  # 像素值为1的坐标列表

    root_pixels = cv2.countNonZero(denoise_image)
    surfArea = root_pixels* pixel_size_mm * pixel_size_mm
    rootFeatures['SurfArea'].append(format(surfArea, '.3g'))

    # 4.根深=竖直方向两端像素点的距离（取根的纵坐标最大值与最小值之差）
    y_coords = [y for y, x in coords_normal]  # 提取所有y坐标
    ...
    x_coords = [x for y, x in coords_normal]  # 提取所有x坐标
    ...
    # # 绘制原始点集
    # plt.plot(coords1[:, 1], coords1[:, 0], 'o')
    #
    # # 绘制凸包的顶点
    # for simplex in hull.simplices:
    #     plt.plot(coords1[simplex, 1], coords1[simplex, 0], 'k-')
    #
    # # 标记凸包的顶点
    # plt.plot(coords1[:, 1], coords1[:, 0], 'ko')
    #
    # # 设置图表标题和标签
